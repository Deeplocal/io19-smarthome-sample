...
SmartHomeModel.getClient = function(clientId, clientSecret) {
  ...
  if (!client || client.clientSecret != clientSecret) {
    console.log(
      "clientSecret doesn't match %s, %s",
      client.clientSecret,
      clientSecret
    );
    return false;
...
Auth.registerAuth = function(app) {
  ...
  app.get("/oauth", function(req, res) {
    ...
    if (authCode) {
      return res.redirect(
        util.format("%s?code=%s&state=%s", redirect_uri, authCode, state)
      );
    }
  ...
  app.post("/login", function(req, res) {
    ...
    if (!user) {
      ...
      return res.redirect(
        util.format(
          ...
          "/frontend",
          req.body.client_id,
          encodeURIComponent(req.body.redirect_uri),
          req.body.state
        )
      );
    }
    ...
    console.log("login successful ", user.name);
    let authCode = SmartHomeModel.generateAuthCode(
      user.uid,
      req.body.client_id
    );
  ...
  app.all("/token", function(req, res) {
    ...
    if ("authorization_code" == grant_type) return handleAuthCode(req, res);
      ...
      return res
        .status(400)
        .send("grant_type " + grant_type + " is not supported");
    }
  });
};
...
function handleAuthCode(req, res) {
  ...
  if (!client) {
    console.error(
      "invalid client id or secret %s, %s",
      client_id,
      client_secret
    );
